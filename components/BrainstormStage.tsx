...
import { Loader2, Zap, TrendingUp, Filter, Clock, X, RotateCcw, Globe, Plus, MapPin, Tag, ChevronDown, ChevronUp, Edit2, Send, MessageSquare, ExternalLink, Sparkles, Lightbulb } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden animate-fadeIn">
      ...
      <div className="bg-white border-b border-slate-200 shadow-sm z-10 flex flex-col md:flex-row">
          ...
          <div className="md:w-1/3 bg-slate-900 p-6 flex flex-col items-center justify-center relative overflow-hidden min-h-[280px]">
              ...
              <div className="relative w-48 h-48 md:w-56 md:h-56 group perspective-1000">
                  ...
                  <svg className="w-full h-full drop-shadow-2xl" viewBox="0 0 100 100">
                       ...
                       {/* Sphere Base */}
                       <circle cx="50" cy="50" r="48" fill="url(#globeGrad)" stroke="#6366f1" strokeWidth="0.5" className="opacity-80" />
                  ...
                  {selectedLocations.map(loc => {
                      ...
                      return (
                          ...
                          >
                              <div className="w-1.5 h-1.5 bg-cyan-400 rounded-full shadow-[0_0_10px_rgba(34,211,238,1)] animate-pulse"></div>
                              <div className="absolute top-3 left-1/2 -translate-x-1/2 bg-slate-900/90 text-cyan-400 border border-cyan-900/50 text-[9px] px-1.5 py-0.5 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap transition-opacity pointer-events-none z-20">
                                  {country.name}
                              </div>
                          </div>
                      );
              ...
              <div className="mt-6 text-center relative z-10">
                  <h3 className="text-white text-xs font-bold tracking-widest flex items-center justify-center gap-2 uppercase">
                      <Globe size={12} className="text-cyan-400" /> Trend Signal Monitor
                  </h3>
                  <div className="flex gap-3 justify-center mt-2 text-[10px] text-slate-400 font-mono">
                      <span className="bg-slate-800/50 px-2 py-0.5 rounded border border-slate-700">{selectedLocations.length} Regions</span>
                      <span className="bg-slate-800/50 px-2 py-0.5 rounded border border-slate-700">{keywords.length} Keywords</span>
                  </div>
              </div>
          </div>
          ...
          <div className="flex-1 p-6 flex flex-col gap-6 overflow-y-auto max-h-[400px] md:max-h-none">
             <div className="flex justify-between items-start">
                 <div>
                     <h2 className="text-2xl font-bold text-slate-900">Configure Signals</h2>
                         ...
                         <div className="flex-[3] pr-4">
                             <div className="flex items-center gap-3 mb-1">
                                ...
                                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border uppercase tracking-wider ${
                                    strategy.automationLevel === 'High' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-amber-200 bg-amber-50 text-amber-700'
                                }`}>
